...
import { Check, CheckCircle, CircleDashed, Edit, FileUp, Loader2, Mail, Phone, ScanLine, Upload, XCircle } from "lucide-react";
...
const Chatbot = () => {
  ...
  useEffect(() => {
    const processStep = () => {
      switch (step) {
          ...
          addMessage(
            <BotMessage key="greeting">
              ...
              <div className="flex gap-2 mt-3">
                <Button onClick={() => handleIdScan('scan')} disabled={actionsDisabled}><ScanLine className="mr-2 h-4 w-4" /> Scan ID Card</Button>
                <Button onClick={() => handleIdScan('upload')} variant="secondary" disabled={actionsDisabled}><Upload className="mr-2 h-4 w-4" /> Upload ID</Button>
              </div>
            </BotMessage>
          );
          break;
        case "ID_SCANNING":
          addMessage(
            <BotMessage key="scanning">
              <div className="flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" /> <p>Scanning ID... Align your card inside the frame.</p></div>
              <div className="mt-2 border-2 border-dashed border-primary rounded-lg p-4 text-center animate-pulse"><p className="text-sm text-gray-500">NIC: 9876543210</p></div>
            </BotMessage>
          );
        ...
        case "CUSTOMER_FOUND":
          addMessage(
            <BotMessage key="customer-found">
              <p className="font-bold text-green-600">Profile found ✅</p>
              <p>You already have an account with us. What would you like to do today?</p>
              ...
              <div className="flex gap-2 mt-3">
                <Button onClick={handleMaintainAccount}><Edit className="mr-2 h-4 w-4" /> Maintain existing account</Button>
                <Button variant="secondary">Open another account</Button>
              </div>
            </BotMessage>
          );
          break;
        case "CUSTOMER_NOT_FOUND":
          addMessage(
            <BotMessage key="customer-not-found">
              <div className="flex items-center gap-2 font-bold text-red-600"><XCircle /> <p>Customer Not Found</p></div>
              <p>We couldn’t find a customer record linked to this NIC.</p>
              <div className="flex flex-wrap gap-2 mt-3"><Button variant="secondary">Open new account</Button><Button onClick={() => { setMessages([]); setStep('GREETING'); }}>Try again</Button><Button variant="outline">Contact Support</Button></div>
            </BotMessage>
          );
          break;
            ...
            <BotMessage key={`profile-edit-${Date.now()}`}>
              ...
              <Card className="mt-3">
                <CardContent className="p-4 space-y-4">
                  <div className="flex justify-between items-center"><div><p className="text-sm text-gray-500">Address</p><p>{updatedData.address}</p></div><Button size="sm" variant="ghost" onClick={() => setStep('UPDATE_ADDRESS')}><Edit className="h-4 w-4" /></Button></div>
                  <Separator />
                  <div className="flex justify-between items-center"><div><p className="text-sm text-gray-500">Contact Details</p><p><Phone className="inline h-4 w-4 mr-2" />{updatedData.mobile}</p><p><Mail className="inline h-4 w-4 mr-2" />{updatedData.email}</p></div><Button size="sm" variant="ghost" onClick={() => setStep('UPDATE_CONTACT')}><Edit className="h-4 w-4" /></Button></div>
                </CardContent>
              </Card>
        ...
        case "ADDRESS_CONFIRM":
          addMessage(
            <BotMessage key="address-confirm">
              ...
              <div className="flex gap-2 mt-3"><Button onClick={handleConfirmAddress}><Check className="mr-2 h-4 w-4" /> Confirm Address</Button></div>
            </BotMessage>
          );
          break;
        case "UPDATE_CONTACT":
        ...
        case "SUCCESS":
          addMessage(
            <BotMessage key="success">
              <p className="font-bold text-lg">All set! 🎉</p><p>Your account details have been updated successfully.</p>
              <div className="flex gap-2 mt-3"><Button onClick={() => window.location.href = '/'}>Finish</Button><Button variant="secondary">View electronic form</Button></div>
            </BotMessage>
          );
          break;
      }
  ...
  const handleIdScan = (method: 'scan' | 'upload') => {
    ...
    addMessage(<UserMessage key="id-scan-action">{method === 'scan' ? 'Scanning ID Card...' : 'Uploading ID Card...'}</UserMessage>);
    setStep('ID_SCANNING');
  };

  const handleMaintainAccount = () => {
    addMessage(<UserMessage key="maintain-action">Maintain existing account</UserMessage>);
    setStep('SHOW_PROFILE_FOR_EDIT');
  };

  const handleUploadBill = () => {
    addMessage(<UserMessage key="upload-bill-action">Uploading UtilityBill_Aug2025.pdf</UserMessage>);
  ...
  const handleConfirmChanges = () => {
    addMessage(<UserMessage key="confirm-changes-action">Confirm Changes</UserMessage>);
    setStep('PROCESSING');
  };

  return (
    <div className="flex flex-col h-screen bg-gray-100 dark:bg-gray-900">
      <header className="p-4 border-b dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm"><h1 className="text-xl font-bold text-center">Account Maintenance</h1></header>
...
const ProcessingMessage = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <BotMessage key="processing">
      <div className="flex items-center gap-2 mb-3"><Loader2 className="h-5 w-5 animate-spin" /><p>Processing your request...</p></div>
      <ul className="space-y-2 text-sm">{items.map((item, i) => <li key={i} className="flex items-center gap-2">{checked[i] ? <CheckCircle className="h-4 w-4 text-green-500" /> : <CircleDashed className="h-4 w-4 text-gray-400 animate-spin" />}<span>{item}</span></li>)}</ul>
    </BotMessage>
  );
};
